...
struct ROI { int x = 0, y = 0, w = 0, h = 0; } g_finalROI;
    ...
    while (true) {
        ...
        if (SUCCEEDED(factory->CreateEncoder(GUID_ContainerFormatPng, nullptr, &encoder))) {
            if (SUCCEEDED(SHCreateStreamOnFileW(task.filePath.c_str(), STGM_CREATE | STGM_WRITE, &stream))) {
                ...
                encoder->Commit();
                stream->Release();
                std::wcout << L"Saved RGB: " << task.filePath << std::endl;
            }
            encoder->Release();
        }
    }
...
// --- Thread 2: Processor (Fixed Channel Swap) ---
void ProcessingWorker(std::wofstream* logFile) {
    while (true) {
        ...
        BYTE* src = raw.bgraData.data();
        BYTE* dst = sTask.rgbData.data();
        for (UINT y = 0; y < sTask.height; ++y) {
            BYTE* sR = src + ((raw.ty + y) * raw.rowPitch) + (raw.tx * 4);
            BYTE* dR = dst + (y * sTask.rowPitch);
            for (UINT x = 0; x < sTask.width; ++x) {
                // To fix the "Orange instead of Blue" bug, we map:
                // Blue  -> Blue (Index 0)
                // Green -> Green (Index 1)
                // Red   -> Red (Index 2)
                dR[x * 3 + 0] = sR[x * 4 + 0];
...
LRESULT CALLBACK SelectorProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    case WM_LBUTTONDOWN: g_ptStartSel = { (int)LOWORD(lParam), (int)HIWORD(lParam) }; g_isDrawingSel = true; SetCapture(hWnd); return 0;
    ...
    case WM_LBUTTONUP:
        if (g_isDrawingSel) {
            ...
            g_finalROI = { min(g_ptStartSel.x, ptEnd.x), min(g_ptStartSel.y, ptEnd.y), abs(ptEnd.x - g_ptStartSel.x), abs(ptEnd.y - g_ptStartSel.y) };
    ...
    case WM_PAINT: {
        ...
        HGDIOBJ hOld = SelectObject(hMem, hbm); HBRUSH hB = CreateSolidBrush(RGB(0, 0, 0));
        FillRect(hMem, &rc, hB); DeleteObject(hB);
        if (g_isDrawingSel) {
            HPEN hP = CreatePen(PS_SOLID, 2, RGB(255, 0, 0)); SelectObject(hMem, hP);
            SelectObject(hMem, GetStockObject(HOLLOW_BRUSH)); POINT cur; GetCursorPos(&cur); ScreenToClient(hWnd, &cur);
            Rectangle(hMem, g_ptStartSel.x, g_ptStartSel.y, cur.x, cur.y); DeleteObject(hP);
        }
        BitBlt(hdc, 0, 0, rc.right, rc.bottom, hMem, 0, 0, SRCCOPY);
        SelectObject(hMem, hOld); DeleteObject(hbm); DeleteDC(hMem); EndPaint(hWnd, &ps); return 0;
    }
...
int main(int argc, char* argv[]) {
    CoInitialize(NULL);
    std::wstring bDir = L"./"; int dSec = 10; float iSec = 2.0f; int mIdx = 0;
    if (argc > 1) {
        int len = MultiByteToWideChar(CP_ACP, 0, argv[1], -1, NULL, 0);
        std::vector<wchar_t> buf(len); MultiByteToWideChar(CP_ACP, 0, argv[1], -1, buf.data(), len);
        bDir = buf.data(); if (!bDir.empty() && bDir.back() != L'\\' && bDir.back() != L'/') bDir += L"\\";
    }
    if (argc > 2) dSec = atoi(argv[2]); if (argc > 3) iSec = (float)atof(argv[3]); if (argc > 4) mIdx = atoi(argv[4]);
    time_t sN = time(0); struct tm sT; localtime_s(&sT, &sN); wchar_t sFN[128];
    swprintf_s(sFN, 128, L"Session_%04d%02d%02d_%02d%02d%02d\\", sT.tm_year + 1900, sT.tm_mon + 1, sT.tm_mday, sT.tm_hour, sT.tm_min, sT.tm_sec);
    wchar_t fBase[MAX_PATH]; if (_wfullpath(fBase, bDir.c_str(), MAX_PATH) == nullptr) wcscpy_s(fBase, MAX_PATH, bDir.c_str());
    g_outputDir = std::wstring(fBase) + L"\\" + sFN;
    ...
    ROI mROI = SelectViewportWithMouse();
    ID3D11Device* dev = nullptr; ID3D11DeviceContext* ctx = nullptr;
    D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, NULL, 0, D3D11_SDK_VERSION, &dev, nullptr, &ctx);
    if (!dev || !ctx) return 1;
    IDXGIDevice* dxDev = nullptr; IDXGIAdapter* adp = nullptr; dev->QueryInterface(__uuidof(IDXGIDevice), (void**)&dxDev);
    ...
    ULONGLONG sT64 = GetTickCount64(); ULONGLONG lS = 0; int fC = 1; ULONGLONG iMS = (ULONGLONG)(iSec * 1000.0f);
    while (((GetTickCount64() - sT64) / 1000) < (ULONGLONG)dSec) {
        ...
        if (SUCCEEDED(dD->AcquireNextFrame(10, &fi, &r)) && r) {
            ...
            if (fC == 1 || (nMS - lS >= iMS)) {
                ...
                if (SUCCEEDED(r->QueryInterface(__uuidof(ID3D11Texture2D), (void**)&gT))) {
                    ...
                    if (SUCCEEDED(dev->CreateTexture2D(&d, nullptr, &st))) {
                        ctx->CopyResource(st, gT); D3D11_MAPPED_SUBRESOURCE m;
                        if (SUCCEEDED(ctx->Map(st, 0, D3D11_MAP_READ, 0, &m))) {
                            RawFrame rf; rf.width = d.Width; rf.height = d.Height; rf.rowPitch = m.RowPitch;
                            rf.tx = (mROI.w > 0) ? mROI.x : 0; rf.ty = (mROI.h > 0) ? mROI.y : 0;
                            rf.tw = (mROI.w > 0) ? mROI.w : d.Width; rf.th = (mROI.h > 0) ? mROI.h : d.Height;
                            rf.timestampMS = nMS; rf.frameIndex = fC++;
                            wchar_t fn[128]; struct tm ti; time_t t = time(0); localtime_s(&ti, &t);
                            swprintf_s(fn, 128, L"%04d_debug_%04d%02d%02d_%02d%02d%02d.png", rf.frameIndex, ti.tm_year + 1900, ti.tm_mon + 1, ti.tm_mday, ti.tm_hour, ti.tm_min, ti.tm_sec);
                            rf.fileName = fn; rf.bgraData.assign((BYTE*)m.pData, (BYTE*)m.pData + (d.Height * m.RowPitch));
                            { std::lock_guard<std::mutex> lk_mtx(g_rawMutex); g_rawQueue.push(std::move(rf)); }
                            g_rawCV.notify_one(); ctx->Unmap(st, 0); lS = nMS;
                        }
                        st->Release();
                    }
                    gT->Release();
    ...
    { std::lock_guard<std::mutex> l1(g_rawMutex); g_running = false; } g_rawCV.notify_all();
    { std::lock_guard<std::mutex> l2(g_saveMutex); } g_saveCV.notify_all();
    if (pT.joinable()) pT.join(); if (iT.joinable()) iT.join();
    log.close(); if (dD) dD->Release(); if (ctx) ctx->Release(); if (dev) dev->Release(); CoUninitialize();
    return 0;
}
